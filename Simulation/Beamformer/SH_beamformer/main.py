...
@references:
    ...
    - [5] A. Koretz and B. Rafaely, "Dolph–chebyshev beampattern design for spherical arrays"
"""
...
az_s, el_s = sphericalBF.sph_sources(L, 'uniform') # Sources DOA
# RANDOM DISTRIBUTION
...
# DIFFERENT ORDERS
# Level constraint in dB
sidelobe_level = utils.dB2linear(-7)
# Crear figura y subplots
fig, axs = plt.subplots(1, 4, figsize=(20, 5), subplot_kw={'projection': 'polar'})
# Plot patterns
for n in range(1, 5):
    ax = axs[n-1]
    cheby_w = sphericalBF.SH_cheby_weights(n, 'sidelobe', sidelobe_level)
    SH_visualization.plotPattern(cheby_w, ax=ax)
    ax.set_title('order = ' + str(n))
    
plt.tight_layout()
plt.show()

# DIFFERENT SIDELOBE LEVELS
# Level constraints in dB
levels_dB = np.array([-7, -5, -3, 0])
levels = utils.dB2linear(levels_dB)
# Creating subplots
fig, axs = plt.subplots(1, 4, figsize=(20, 5), subplot_kw={'projection': 'polar'})
# Plot patterns
for i, level in enumerate(levels):
    ax = axs[i]
    cheby_w = sphericalBF.SH_cheby_weights(4, 'sidelobe', level)
    SH_visualization.plotPattern(cheby_w, ax=ax)
    ax.set_title('sidelobe level = ' + str(levels_dB[i]) + 'dB')
