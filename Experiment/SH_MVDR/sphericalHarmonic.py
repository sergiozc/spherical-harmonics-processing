...
class SHutils:
    ...
    def signal2SH(signal, order, el, az):
        ...
        SH_signal = signal @ Y_N.T
        
        return SH_signal
    
    @staticmethod
    def transform_SH(p, el, az, order):
        """
        Calculates spherical harmonics coefficients.
        
        Parameters:
            p(numpy.ndarray): sound pressure from mics (samples x Nmics)
            el (numpy.ndarray): Vector of elevation (mics) angles in radians.
            az (numpy.ndarray): Vector of azimuth (mics) angles in radians.
            order (int): Maximum order of the spherical harmonics.
        
        Returns:
            pnm(numpy.ndarray): Sound pressure in SH domain
        """
        num_samples, num_mics = p.shape
        Y_N = SHutils.realSH(order, el, az)
        N_harm = N_harm = (order + 1) ** 2
        pnm = np.zeros((N_harm, num_samples))
        
        for i in range(num_samples):
            pnm[:, i] = Y_N @ p[i,:] # Per sample
